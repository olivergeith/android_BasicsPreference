...
public interface URLHandler {
	...
	/**
	 * Using the slower REQUEST method for getting the basic URL infos. Use this when getting errors behind a
	 * problematic/special proxy or firewall chain.
	 */
	...
	public static class URLInfo {
		...
		private final String bodyCharset;

		protected URLInfo(final boolean available, final long contentLength, final long lastModified) {
			this(available, contentLength, lastModified, null);
		}

		protected URLInfo(final boolean available, final long contentLength, final long lastModified,
				final String bodyCharset) {
	 ...
	 *            the url to check
	 * @param timeout
	 *            the maximum time before considering an url is not reachable a timeout of zero indicates no timeout
	 * @return the length of the target if available, 0 if not reachable
	 */
	...
	/**
	 * Returns the URLInfo of the given url or a {@link #UNAVAILABLE} instance, if the url is not reachable.
	 * 
	 * @param url
	 *            The url from which information is retrieved.
	 * @return The URLInfo extracted from the given url, or {@link #UNAVAILABLE} when the url is not available.
	 */
