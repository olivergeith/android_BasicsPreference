...
import de.geithonline.android.basics.utils.ivy.URLHandler.URLInfo;
...
public class ApacheURLLister {
	...
	private static final Pattern PATTERN = Pattern.compile(
			"<a[^>]*href=\"([^\"]*)\"[^>]*>(?:<[^>]+>)*?([^<>]+?)(?:<[^>]+>)*?</a>", Pattern.CASE_INSENSITIVE);
	 ...
	 */
	public List<URL> listAll(final URL url) throws IOException {
	 ...
	 *             If an error occures retrieving the HTML.
	 */
	public List<URL> listDirectories(final URL url) throws IOException {
	 ...
	 *             If an error occures retrieving the HTML.
	 */
	public List<URL> listFiles(final URL url) throws IOException {
	 ...
	 *             If an error occures retrieving the HTML.
	 */
	public List<URL> retrieveListing(URL url, final boolean includeFiles, final boolean includeDirectories)
			throws IOException {
		...
		final URLHandler urlHandler = URLHandlerRegistry.getDefault();
		final URLInfo urlInfo = urlHandler.getURLInfo(url);
		if (urlInfo == URLHandler.UNAVAILABLE) {
			return urlList; // not found => return empty list
		}
		// here, urlInfo is valid
		final String charset = urlInfo.getBodyCharset();

		final InputStream contentStream = urlHandler.openStream(url);
		...
		final String htmlText = FileUtil.readEntirely(r);
		...
		while (matcher.find()) {
			...
			if (href.startsWith("/")) {
				final int slashIndex = href.substring(0, href.length() - 1).lastIndexOf('/');
			...
			if (text.endsWith("..>")) {
				...
				// trailing slashes
				final String strippedHref = href.endsWith("/") ? href.substring(0, href.length() - 1) : href;
				final String strippedText = text.endsWith("/") ? text.substring(0, text.length() - 1) : text;
				if (!strippedHref.equalsIgnoreCase(strippedText)) {
					continue;
				}
			}

			final boolean directory = href.endsWith("/");

			if ((directory && includeDirectories) || (!directory && includeFiles)) {
				final URL child = new URL(url, href);
				urlList.add(child);
				Log.d("ApacheURLLister", "found URL=[" + child + "].");
			}
		}
