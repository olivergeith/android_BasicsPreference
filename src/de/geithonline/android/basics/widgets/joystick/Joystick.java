package de.geithonline.android.basics.widgets.joystick;
...
/**
 * A simple and flexible joystick. Extends FrameLayout and should host one direct child to act as the draggable stick. Use
 * {@link #setJoystickListener(JoystickListener)} to observe user inputs.
 */
public class Joystick extends FrameLayout {
	...
	public Joystick(final Context context, final AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs);
	}

	public Joystick(final Context context, final AttributeSet attrs, final int defStyleAttr) {
	...
	public Joystick(final Context context, final AttributeSet attrs, final int defStyleAttr, final int defStyleRes) {
	...
	private void init(final Context context, final AttributeSet attrs) {
		...
		if (null != attrs) {
			final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.Joystick);
			...
			motionConstraint = MotionConstraint.values()[a.getInt(R.styleable.Joystick_motion_constraint, motionConstraint.ordinal())];
			a.recycle();
		}
	}
	...
	private void recalculateRadius(final int width, final int height) {
	...
	public void setJoystickListener(final JoystickListener listener) {
		...
		if (!hasStick()) {
			Log.w(LOG_TAG, LOG_TAG + " has no draggable stick, and is therefore not functional. " + "Consider adding a child view to act as the stick.");
		}
	}

	/**
	 * Locks the stick position when next the user releases it. Note that {@link JoystickListener#onUp()} will not be called upon release. Resets to unlocked
	 * state after subsequent touch.
	 */
	@SuppressWarnings("unused")
	public void lock() {
		locked = true;
	...
	/**
	 * @return Distance in pixels a touch can wander before the joystick thinks the user is manipulating the stick.
	 */
	@SuppressWarnings("unused")
	...
	/**
	 * @param touchSlop
	 *            Distance in pixels a touch can wander before the joystick thinks the user is manipulating the stick.
	 */
	@SuppressWarnings("unused")
	public void setTouchSlop(final int touchSlop) {
	...
	@SuppressWarnings("unused")
	public void setMotionConstraint(final MotionConstraint motionConstraint) {
		this.motionConstraint = motionConstraint;

		if (!hasFixedRadius) {
			recalculateRadius(getWidth(), getHeight());
		}
	}
	...
	/**
	 * @param radius
	 *            The maximum offset in pixels from the center that the stick is allowed to move.
	 */
	@SuppressWarnings("unused")
	...
	/**
	 * @param startOnFirstTouch
	 *            If true, the stick activates immediately on the initial touch. Else, the user must begin to drag their finger across the joystick for the
	 *            stick to activate.
	 */
	@SuppressWarnings("unused")
	public void setStartOnFirstTouch(final boolean startOnFirstTouch) {
	...
	public boolean onTouchEvent(@NonNull final MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			...
			if (dragInProgress) {
				...
				final float latestX = event.getX(pointerIndex);
				final float latestY = event.getY(pointerIndex);

				final float deltaX = latestX - downX;
				final float deltaY = latestY - downY;
	...
	private boolean dragExceedsSlop(final MotionEvent event) {
		final int pointerIndex = event.findPointerIndex(activePointerId);
	...
	private void onDragStop() {
		...
		if (!locked) {
			draggedChild.animate().translationX(0).translationY(0).setDuration(STICK_SETTLE_DURATION_MS).setInterpolator(STICK_SETTLE_INTERPOLATOR).start();
		}
	 ...
	 */
	private void onDrag(final float dx, final float dy) {
	...
	public LayoutParams generateLayoutParams(final AttributeSet attrs) {
		final LayoutParams params = new LayoutParams(getContext(), attrs);
	...
	protected ViewGroup.LayoutParams generateLayoutParams(@NonNull final ViewGroup.LayoutParams p) {
		final LayoutParams params = new LayoutParams(p);
		params.gravity = Gravity.CENTER;
		return params;
	}
