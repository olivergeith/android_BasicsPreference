...
package de.geithonline.android.basics.widgets.rangeseekbar;
...
/**
 * Widget that lets users select a minimum and maximum value on a given numerical range. The range value types can be one of Long, Double, Integer, Float,
 * Short, Byte or BigDecimal.<br>
 * <br>
...
public class RangeSeekBar<T extends Number>extends ImageView {
    ...
    private static final int LINE_HEIGHT_IN_DP = 8;
    ...
    private final Path translatedThumbShadowPath = new Path();
    ...
    private T extractNumericValueFromAttributes(final TypedArray a, final int attribute, final int defaultValue) {
        final TypedValue tv = a.peekValue(attribute);
        if (tv == null) {
            return (T) Integer.valueOf(defaultValue);
        }

        final int type = tv.type;
    ...
    private void init(final Context context, final AttributeSet attrs) {
        float barHeight;
        final int thumbNormal = R.drawable.seek_thumb_normal_small;
        final int thumbPressed = R.drawable.seek_thumb_pressed_small;
        final int thumbDisabled = R.drawable.seek_thumb_disabled_small;
        int thumbShadowColor;
        final int defaultShadowColor = Color.argb(75, 0, 0, 0);
        final int defaultShadowYOffset = PixelUtil.dpToPx(context, 2);
        final int defaultShadowXOffset = PixelUtil.dpToPx(context, 0);
        final int defaultShadowBlur = PixelUtil.dpToPx(context, 2);
        ...
        if (attrs == null) {
            ...
            try {
                setRangeValues(extractNumericValueFromAttributes(a, R.styleable.RangeSeekBar_absoluteMinValue, DEFAULT_MINIMUM),
                ...
                final Drawable normalDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbNormal);
                if (normalDrawable != null) {
                    thumbImage = BitmapUtil.drawableToBitmap(normalDrawable);
                }
                final Drawable disabledDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbDisabled);
                if (disabledDrawable != null) {
                    thumbDisabledImage = BitmapUtil.drawableToBitmap(disabledDrawable);
                }
                final Drawable pressedDrawable = a.getDrawable(R.styleable.RangeSeekBar_thumbPressed);
        ...
        textOffset = !showTextAboveThumbs ? 0 : this.textSize + PixelUtil.dpToPx(context, DEFAULT_TEXT_DISTANCE_TO_BUTTON_IN_DP) + this.distanceToTop;

        rect = new RectF(padding, textOffset + thumbHalfHeight - barHeight / 2, getWidth() - padding, textOffset + thumbHalfHeight + barHeight / 2);
        ...
        if (thumbShadow) {
            ...
            thumbShadowPath.addCircle(0, 0, thumbHalfHeight, Path.Direction.CW);
        }
    }

    public void setRangeValues(final T minValue, final T maxValue) {
        this.absoluteMinValue = minValue;
        this.absoluteMaxValue = maxValue;
        setValuePrimAndNumberType();
    }

    public void setRangeValues(final T minValue, final T maxValue, final T step) {
        this.absoluteStepValue = step;
        setRangeValues(minValue, maxValue);
    }

    public void setTextAboveThumbsColor(final int textAboveThumbsColor) {
        this.textAboveThumbsColor = textAboveThumbsColor;
        invalidate();
    }

    public void setTextAboveThumbsColorResource(@ColorRes final int resId) {
     ...
     */
    @SuppressWarnings("unchecked")
    private T roundOffValueToStep(final T value) {
        final double d = Math.round(value.doubleValue() / absoluteStepValuePrim) * absoluteStepValuePrim;
     ...
     */
    public void setSelectedMinValue(final T value) {
     ...
     */
    public void setSelectedMaxValue(final T value) {
        // in case absoluteMinValue == absoluteMaxValue, avoid division by zero when normalizing.
        if (0 == (absoluteMaxValuePrim - absoluteMinValuePrim)) {
    ...
    /**
     * Set the path that defines the shadow of the thumb. This path should be defined assuming that the center of the shadow is at the top left corner (0,0) of
     * the canvas. The {@link #drawThumbShadow(float, Canvas)} method will place the shadow appropriately.
     *
     * @param thumbShadowPath
     *            The path defining the thumb shadow
     */
    public void setThumbShadowPath(final Path thumbShadowPath) {
    ...
    protected synchronized void onMeasure(final int widthMeasureSpec, final int heightMeasureSpec) {
        ...
        int height = thumbImage.getHeight() + (!showTextAboveThumbs ? 0 : PixelUtil.dpToPx(getContext(), HEIGHT_IN_DP))
    ...
    protected synchronized void onDraw(@NonNull final Canvas canvas) {
        ...
        if (showLabels) {
            ...
            // minLabelSize = paint.measureText(minLabel);
            // maxLabelSize = paint.measureText(maxLabel);
        ...
        final boolean selectedValuesAreDefault = (normalizedMinValue <= minDeltaForDefault && normalizedMaxValue >= 1 - minDeltaForDefault);

        final int colorToUseForButtonsAndHighlightedLine = //
                !alwaysActive && !activateOnDefaultValues && selectedValuesAreDefault ? defaultColor : // default
        ...
        if (showTextAboveThumbs /* && (activateOnDefaultValues || !selectedValuesAreDefault) */) {
            ...
            final String minText = valueToString(getSelectedMinValue());
            final String maxText = valueToString(getSelectedMaxValue());
    ...
    protected String valueToString(final T value) {
        return String.valueOf(value);
    }

    /**
     * Overridden to save instance state when device orientation changes. This method is called automatically if you assign an id to the RangeSeekBar widget
     * using the {@link #setId(int)} method. Other members of this class than the normalized min and max values don't need to be saved.
     */
    @Override
    ...
    /**
     * Overridden to restore instance state when device orientation changes. This method is called automatically if you assign an id to the RangeSeekBar widget
     * using the {@link #setId(int)} method.
     */
    @Override
     ...
     */
    private void drawThumb(final float screenCoord, final boolean pressed, final Canvas canvas, final boolean areSelectedValuesDefault) {
        Bitmap buttonToDraw;
        ...
        BitmapHelper.tintImage(buttonToDraw, paint, activeColor);
     ...
     *
     * @param screenCoord
     *            the x-coordinate of the slider thumb
     * @param canvas
     *            the canvas on which to draw the shadow
     */
    private void drawThumbShadow(final float screenCoord, final Canvas canvas) {
     ...
     */
    private Thumb evalPressedThumb(final float touchX) {
        Thumb result = null;
        ...
        if (minThumbPressed && maxThumbPressed) {
            // if both thumbs are pressed (they lie on top of each other), choose the one with more room to drag. this avoids "stalling" the thumbs in a corner,
     ...
     */
    private boolean isInThumbRange(final float touchX, final double normalizedThumbValue) {
     ...
     */
    private void setNormalizedMinValue(final double value) {
     ...
     */
    private void setNormalizedMaxValue(final double value) {
     ...
     */
    @SuppressWarnings("unchecked")
    protected T normalizedToValue(final double normalized) {
        final double v = absoluteMinValuePrim + normalized * (absoluteMaxValuePrim - absoluteMinValuePrim);
     ...
     */
    protected double valueToNormalized(final T value) {
        if (0 == absoluteMaxValuePrim - absoluteMinValuePrim) {
    ...
    private double screenToNormalized(final float screenCoord) {
        ...
        if (width <= 2 * padding) {
            ...
            final double result = (screenCoord - padding) / (width - 2 * padding);
    ...
    protected enum NumberType {
        ...
        public static <E extends Number> NumberType fromNumber(final E value) throws IllegalArgumentException {
